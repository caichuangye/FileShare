...
public class ImageAdapter extends FileBaseAdapter<ImageItem> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(mMode == GlobalParams.SERVER_MODE){
            path = "file://" + CommonUtil.getLocalPath(item.getPath());
        }else{
            path = "file://" + item.getPath();
        }
        HLog.d(getClass(),HLog.L,"path = "+path);
        ImageLoader.getInstance().displayImage(path, aware, mOptions, new ImageSize(174 * 3, 174 * 3), null, null);
        ...
        if (mMode == GlobalParams.SERVER_MODE) {
            ...
            if (status != DownloadStatus.DOWNLOADING) {
                ...
                holder.downloadTextView.setStatus(ShareApplication.getInstance().getDownloadPercent(item.getPath(),mServerIP));
            }
